---
name: lit-review-protocol
description: Four-phase agent-assisted literature review producing a Zotero-compatible bibliography and per-source classification mapped to the paper's sections; composes with project-setup.
audience: public
version: 0.1
---
...
Governing principles: **user control** and **radical transparency** — see {{LIBRARY_ROOT}}/skills/about-governing-principles.md. The agent **never** decides on its own what to research, how broadly to scope, or how to rank sources. Every phase begins with an interview that gets explicit user sign-off before the agent commits anything to disk.
...
6. `human-inputs/` — user-authored material: the user's notes, annotated PDFs, Scholar exports, colleague recs, Zotero BibTeX exports. `human-verified` by authorship (upstream sources still need independent verification).
...
1. **Interview first, act second.** Every phase below has a required interview checklist. Do not move to the *action* step of a phase until the user has answered the interview questions.
2. **Propose, don't decide.** When the agent has ideas (candidate strands, candidate citekeys, candidate source tiers), present them as a numbered list with clear "approve / edit / reject" options. Wait for the user's choice.
...
Goal: turn the paper's outline into a short, named list of literature strands, approved by the user.
...
Present to the user for approval / edits / additions / removals. Iterate until signed off. Save to `lit-review-strategy.md` only after sign-off.
...
Goal: convert agent + user output into a single `references.bib` plus a parallel `classification.csv` row per source.
...
1. Normalize each incoming source:
   ...
   - Dedupe by DOI / title against existing `references.bib`. Ask the user how to resolve conflicts.
2. Append verified entries to `references.bib`, keeping the file sorted by citekey.
3. Add a row to `classification.csv` for each new source. Fill `strand`, `evidence_type`, `reliability`, `sections` from the agent output — but tag `read_status = unread` and `verification = partially-verified` until the user reads.
...
Goal: every BibTeX entry has a complete `classification.csv` row. High-reliability sources and sources the user plans to cite get a deep note. The user can see at a glance where each source is going in the paper.
...
1. For each new entry added in Phase 3, present the agent's proposed classification (strand, sections, evidence type, reliability) to the user for approval or edit.
2. Once approved, `verification` stays at `partially-verified` until the user reads and marks `read_status = read`.
3. For sources the user plans to cite or quote, generate `<citekey>.md` with: thesis, method, key evidence, limitations, quote candidates, usable in which paper section.
...
- New sources found mid-drafting → run through phases 3–4 directly; do not open a new strand unless the user approves.
...
No user-facing importance tier column. Importance emerges from `sections` (where the source will be cited) and `read_status` (how deeply it has been engaged with).
...
- A paper-writing protocol the user maintains can consume `references.bib` + `classification.csv` when drafting.
...
- **Silent classification.** The agent quietly tagging sources is the opposite of this protocol's intent — every classification decision surfaces to the user.
