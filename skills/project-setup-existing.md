...
description: Companion to `project-setup.md` for retrofitting agentic workflow into a project that already has its own folder structure. Inventories existing state, presents three retrofit depths (full restructure / selective additions / minimal adoption), produces a sign-off plan, executes it, backfills the asset registry, wires the skill library, and registers the project in the active ledger.
audience: public
version: 0.1
---
...
- The user wants to adopt agentic workflow on a project that already has its own folder structure, naming conventions, and accumulated assets.
...
- The user has not yet decided whether they want to adopt the workflow at all — in that case, walk them through the full `project-setup.md` template first, then return here to decide on depth.

## Artifacts the skill produces

1. `existing-structure.md` (temporary, in a scratch location the user approves) — the inventory produced in Phase 1.
...
3. Depending on the depth chosen in Phase 2:
   ...
   - **Option C (Minimal adoption):** one agent-facing folder (default name `agent-workspace/`, user-renameable) at project root, plus `asset-registry.csv` and `interaction-log.csv` at project root, plus a short `README.md` section describing the adoption.
...
2. **Three options, not one.** Always present all three retrofit depths with their trade-offs, even when one looks obviously right. The user's call.
...
5. **Backfill conservatively.** When populating `asset-registry.csv` for pre-existing assets, default `verification = not-verified` or `partially-verified`. Never assign `human-verified` to a file the user has not re-read in this session.
...
7. **Minimal first, restructure last.** When in doubt, prefer the lowest-impact option that closes the gap the user named. Restructures are expensive and their benefits compound slowly.
...
5. *"Which category does this project belong to in the active ledger — `research-project`, `public-writing`, `teaching-project`, `business`, or `personal` (adapt to your ledger's category list)? And what one-word subtype best describes it (e.g., `paper`, `book`, `op-ed`, `course`, `consulting`, `website`)?"*

**Action step:** if the project is near-empty or the user has not yet committed to agentic workflow, hand off to `project-setup.md` and stop. Otherwise, write a one-paragraph scope statement naming the project, the trigger, and any external sync constraints. Get sign-off before continuing.
...
Present the inventory back to the user for correction. Do not move to Phase 2 until they have confirmed the map is accurate.

---

## Phase 2 — Present the three retrofit options

Goal: give the user a real choice of depth, with honest trade-offs.

**Action step:** write each option as a short proposal the user can read in a minute. Template for each:
...
- **When it makes sense:** the project is early enough that external sync is shallow, the user wants this project to be a model of the workflow, the team benefits from a shared canonical layout.
- **Cost:** one to several days of attention; every external sync dependency has to be updated; git history gets a large rename commit that can complicate blame.
- **Risk:** highest. One broken path in Overleaf or a script is enough to stall work.

### Option B — Selective additions

Keep the existing layout. Add only the template folders whose absence is actually costing the user something — e.g., add `background/literature/` if literature is currently scattered, or add `appendix/` only when the project is approaching submission. Always add the two ledgers and a filled-in `README.md` that names the hybrid layout.

- **When it makes sense:** the project has a working layout the user is fluent in, but a few specific gaps are real.
...
1. One agent-facing folder (default `agent-workspace/`, or whatever the user prefers) — scratch space for agent outputs, plans, intermediate notes, so agents never write into the user's working folders by surprise.
2. `asset-registry.csv` — schema per `project-setup.md`. Backfilled only to the extent the user wants; at minimum, populated going forward.
...
- **When it makes sense:** the project layout works, outside-system dependencies are thick, the user wants provenance tracking without paying the cost of a restructure.
- **Cost:** under an hour.
- **Risk:** lowest.
...
- `LEAVE <path>` for anything explicitly outside the retrofit (useful when the user has flagged a no-touch zone).

Read the plan back to the user. Ask: *"Any items to add, remove, or reorder? Any that should wait for a separate session?"* Do not proceed until the user gives explicit sign-off on the final list.
...
2. After each external-sync-affecting move, pause and ask the user to verify the downstream system still works before continuing.
...
4. When the plan finishes, produce a short "what changed" summary for the user: list the moves executed, the files created, and anything skipped.

---

## Phase 5 — Backfill the asset registry

Goal: populate `asset-registry.csv` with pre-existing assets at a level of fidelity the user chooses.
...
3. *"What default for `verification` — `not-verified` for everything, or `partially-verified` for anything you have previously reviewed?"*

**Action step:** append rows in bulk per the user's choices. For any row where `creator` or `model_metadata` is genuinely unknown, leave the cell blank rather than guess. Flag the blank rows in the summary so the user can fill them opportunistically.
...
1. If the user has no skill library, dispatch to `skills-library-setup.md` as a subagent first.
...
- `skills-library-setup.md`: called from Phase 6 when the user does not yet have a skill library.
...
- `lit-review-protocol.md`: if Phase 1 reveals literature scattered across the project, the user may want to consolidate through a lit-review pass — that is a separate session, not part of the retrofit.

## Common failure modes

- **Restructure by default.** The agent proposes Option A because it looks tidiest, and the user accepts before understanding the external-sync cost. Symptom: the week after retrofit, a bibliography path breaks or an Overleaf `\input` fails. Recovery: default to Option C in the presentation, and require the user to argue up to B or A.
...
- **Aggressive backfill verification.** The agent marks old files `human-verified` because the user produced them. Symptom: downstream work trusts a claim the user has not re-read in a year. Recovery: rewrite all backfilled `verification` cells to `not-verified` or `partially-verified` unless the user explicitly re-reviewed the file in this session.
- **Silent deletion.** A displaced file is deleted rather than archived because it "looks redundant." Symptom: the user can't find a note they remembered writing. Recovery: restore from the nearest backup; add the archive rule to the project's `Rules for Agents` block.
...
A user has been working on a diff-in-differences empirical project for eighteen months. Folder layout: `code/`, `data/`, `writeup/`, `misc/`. Overleaf syncs to `writeup/`. A co-author has a local clone with the same paths. Goal: bring this project under agentic workflow before a replication-package handoff three months out.

1. **Phase 0** — the user confirms the project is active, names the Overleaf sync and the co-author clone as external dependencies, and names the replication package as the trigger.
...
3. **Phase 2** — the user picks Option B (selective additions) because `code/` and `data/` should not be renamed (co-author sync) but the project lacks a literature folder, an appendix, and the two ledgers.
...
6. **Phase 5** — registry backfilled for `writeup/` drafts and `background/literature/` PDFs only, per the user's request; `code/` and `data/` will be backfilled opportunistically over the next month.
7. **Phase 6** — the existing skill library is at `{{LIBRARY_ROOT}}`; a "Skills" section is added to the new `README.md` pointing there.

Total time: about two hours of user attention, no external sync broken, registry populated enough to support the three-month push to submission.
