---
name: skill-writing-protocol
description: Six-phase interview-driven procedure for writing a new skill — combines the user's existing practice with external sources to produce a reusable, named playbook that any agent instance can invoke.
audience: public
version: 0.1
---
...
A six-phase skill for writing a new skill: an interview-driven procedure that combines the user's existing practice with external sources to produce a reusable, named playbook that any agent instance can invoke.
...
Governing principles: **user control** and **radical transparency** — see {{LIBRARY_ROOT}}/skills/about-governing-principles.md. The agent never decides on its own what the skill should prescribe; the skill encodes the user's practice, not the agent's training-memory average. Every phase begins with an interview and waits for explicit sign-off before committing anything to disk.

---

## When to use

- A recurring task keeps being re-specified from scratch in every new session.
- An existing skill is showing its age: the assumptions behind it have changed, or current best practice has moved beyond it.
- The user wants to institutionalize a preferred method so that multiple agent instances and subagents apply it consistently.
- A team or lab wants to share a common practice across projects and collaborators.

Do **not** start a new skill when the task is genuinely one-off, when an existing skill would cover the case with a small edit, or when the user has not yet formed a clear preference about how the task should be done. Skills are load-bearing documents; they should not be written speculatively.

## Artifacts the protocol produces

Under `{{LIBRARY_ROOT}}/skills/` (global library) or `appendix/` of the active project (local, replication-bound):
...
1. **Interview first, draft second.** Every phase below has an interview step. Do not move to the drafting step of a phase until the user has answered the questions.
2. **Don't invent preferences.** If the user has not expressed a preference on a design question, mark the question `OPEN` in the draft and return to it. Do not fill gaps with plausible-sounding defaults.
3. **Mine artifacts, don't mine training memory.** The skill should encode this user's practice as evidenced in their own prior work, not the genre-average practice the model has seen during training.
...
6. **Invoke the security officer when nervous.** If a phase you are about to execute could mislead the user, harm the project, or commit them to a position they have not framed, invoke `security-officer` (sentinel `[security-officer: <reason>]` or `Agent({subagent_type: "security-officer", ...})`) before acting. The officer is diagnostic and advisory; you keep authorship. See `{{LIBRARY_ROOT}}/skills/security-officer-protocol.md`.
...
Goal: extract the user's existing approach from things they have already produced.
...
- Recurring moves (what does the user always do?)
- Recurring decisions (what choices come up every time?)
- Implicit rules (what does the user never do, even when it would be natural?)
- Worked example (one or two past artifacts annotated by the extraction agent)

Present the extraction back to the user for correction, before proceeding to Phase 2.

---

## Phase 2 — Gather background sources

Goal: ground the skill in external knowledge where that knowledge is load-bearing, without diluting the user's own practice with genre-average text.
...
Keep extracted external rules clearly distinguishable from the user's own practice. Internal practice is the source of truth; external sources are the scaffolding.
...
3. *"When this skill is invoked, when should the agent pause and ask the user, and when should it proceed?"*
...
**Action step:** revise the scope statement, the rule list, and the anti-patterns list with the user's answers.
...
Write the skill in the second-person imperative ("ask the user...", "do not commit..."), not the first-person descriptive ("I will ask..."). The voice is that of a protocol, not a narrator.
...
2. Log where the agent paused unnecessarily, where it should have paused and did not, where the prescribed rules produced friction, and where the user had to manually intervene.
...
- `lit-review-protocol.md`: called as a subagent during Phase 2 when the skill needs external grounding beyond the user's anchor sources.
- A paper-writing protocol the user maintains: consumed by skills that produce prose artifacts (to match voice and style).
- Any future skill: may itself call this skill-writing protocol when it is first drafted.

## Common failure modes

- **Training-memory skills.** The skill reads as a textbook summary rather than as this user's practice. Symptom: rules are generic and could apply to anyone in the field. Recovery: rerun Phase 1 with more artifacts; cut rules that are not traceable to a specific artifact or a specific answer from Phase 3.
- **Over-broad scope.** The skill tries to cover every adjacent case and ends up under-specifying each. Symptom: every invocation requires the user to re-scope the task. Recovery: split the skill into two or three narrower skills.
